...
namespace Brubeck.Assembler
{
	...
	{
		/// <summary>
		/// Path of the current file being assembled.
		/// </summary>
		public string AsmPath { get; private set; }

		/// <summary>
		/// Verbosity of the current assembly process.
		/// </summary>
		public bool AsmVerbose { get; private set; }
		...
		internal Mnemonic[] Run(string path, bool verbose = false, bool nested = false)
		{
			...
			try
			{
				...
				foreach(string cmd in code)
				{
					if(cmd[0] == '#')
                    {
						commands.AddRange(InterpreterCommands.RunCommand(this, cmd));
                    }
					else
                    {
				...
				if(requiresaddressresolve && !nested)
				{
					Console.WriteLine("\nNamed Locations:");
			...
			return Array.Empty<Mnemonic>();
		}

		public void Invoke(string path, bool verbose = false)
        {
			Run(path, verbose);
        }
	}
